/**
 * Universal Clipboard Sync – Secure Server (LOCAL)
 *
 * - Text and image clipboard sync only (file support removed)
 * - E2EE-safe relay
 * - Offline queue for text messages
 */
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", async (raw) => {
    ...
    /* -------------------------------------------------
       CLIPBOARD UPDATE (TEXT + IMAGE ONLY)
    ------------------------------------------------- */
